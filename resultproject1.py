...
class WeatherDiary:
    ...
    def create_widgets(self):
        # Поля ввода
        ttk.Label(self.root, text="Дата (YYYY-MM-DD):").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.date_entry = ttk.Entry(self.root)
        self.date_entry.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(self.root, text="Температура (°C):").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.temp_entry = ttk.Entry(self.root)
        self.temp_entry.grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(self.root, text="Описание погоды:").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        self.desc_entry = ttk.Entry(self.root)
        self.desc_entry.grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(self.root, text="Осадки:").grid(row=3, column=0, sticky="w", padx=5, pady=5)
        self.precip_var = tk.BooleanVar()
        ttk.Checkbutton(self.root, variable=self.precip_var).grid(row=3, column=1, sticky="w")

        # Кнопка добавления
        ttk.Button(self.root, text="Добавить запись", command=self.add_entry).grid(row=4, column=0, columnspan=2, pady=10)
        ...
        # Фильтры
        ttk.Label(self.root, text="Фильтр по дате:").grid(row=6, column=0, sticky="w", padx=5, pady=5)
        self.filter_date_entry = ttk.Entry(self.root)
        self.filter_date_entry.grid(row=6, column=1, padx=5, pady=5)

        ttk.Label(self.root, text="Фильтр по температуре (>):").grid(row=7, column=0, sticky="w", padx=5, pady=5)
        ...
        ttk.Button(self.root, text="Сохранить в JSON", command=self.save_data).grid(row=9, column=0, pady=10)
        ttk.Button(self.root, text="Загрузить из JSON", command=self.load_data).grid(row=9, column=1, pady=10)
    ...
    def update_table(self, entries=None):
        ...
        entries_to_show = entries if entries is not None else self.entries
    ...
    def load_data(self):
        try:
            ...
            self.entries = []
            messagebox.showwarning("Предупреждение", "Файл данных не найден. Создан новый.")
